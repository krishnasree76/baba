...
export default function Addresses() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingId 
      ? `http://127.0.0.1:8000/api/auth/addresses/${editingId}/` 
  ...
  const handleDelete = async (id: number) => {
    ...
    const res = await fetch(`http://127.0.0.1:8000/api/auth/addresses/${id}/`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    <div className="bg-baba-softbg min-h-screen py-16 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-12 text-center sm:text-left">
          <div>
            <h1 className="text-4xl font-serif font-bold text-baba-primary tracking-tight">Saved Addresses</h1>
            <p className="text-gray-500 mt-2">Update your delivery destinations for wholesale orders</p>
          </div>
          <button
            onClick={() => {
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1">
                  ...
                  <input name="name" value={form.name} onChange={handleChange} className="w-full border-b border-gray-200 py-3 focus:border-baba-accent outline-none transition-colors" required />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Contact Number</label>
                  <input name="phone" value={form.phone} onChange={handleChange} className="w-full border-b border-gray-200 py-3 focus:border-baba-accent outline-none transition-colors" required />
                </div>
                <div className="md:col-span-2 space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Street / Area / Landmark</label>
                  <input name="street" value={form.street} onChange={handleChange} className="w-full border-b border-gray-200 py-3 focus:border-baba-accent outline-none transition-colors" required />
                </div>
                <div className="grid grid-cols-3 md:col-span-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input name="city" value={form.city} onChange={handleChange} className="w-full border-b border-gray-200 py-3 focus:border-baba-accent outline-none transition-colors" required />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">State</label>
                    <input name="state" value={form.state} onChange={handleChange} className="w-full border-b border-gray-200 py-3 focus:border-baba-accent outline-none transition-colors" required />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">PIN</label>
                    <input name="pincode" value={form.pincode} onChange={handleChange} className="w-full border-b border-gray-200 py-3 focus:border-baba-accent outline-none transition-colors" required />
                  </div>
                </div>
              </div>
        ...
        <div className="flex flex-col gap-6">
          {addresses.length === 0 && !showForm && (
            <div className="py-24 text-center bg-white rounded-2xl border-2 border-dashed border-gray-200">
              ...
              <p className="text-gray-400 font-serif text-lg italic">No shipping addresses saved yet.</p>
            </div>
          )}
          ...
          {addresses.map((addr) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="font-serif text-2xl font-bold text-baba-primary">{addr.name}</span>
              </div>
              ...
              <div className="text-lg text-gray-600 space-y-2 ml-12">
                ...
                <div className="flex items-center gap-3 mt-8 pt-6 border-t border-gray-50">
                  ...
                  <span className="font-mono text-baba-primary font-bold tracking-wider">{addr.phone}</span>
                </div>
              </div>
            </motion.div>
          ))}
